...
# 0. 环境设置与路径
# ==========================================
output_dir = 'Results_Great/Hazard_Mechanism_Altitude'
...
def adaptive_discretization(series, col_name, n_bins=5):
    ...
    # 修复2：检查分布是否极端不均衡（某个值占比>95%）
    value_counts = series.value_counts(normalize=True)
    if value_counts.iloc[0] > 0.95:
        # 某个值占比超过95%，分布极端不均衡，保持原样
        return series
...
for i, (ele_val, ele_name) in enumerate(ele_map.items()):
    ...
    rank_df['Altitude'] = ele_name
    all_altitude_q.append(rank_df)
    
    # 5. 绘图
    sns.barplot(x='q', y='Feature', data=rank_df, ax=axes[i], palette='magma')
    axes[i].set_title(f'Altitude: {ele_name} (n={len(df_sub)})', fontsize=20)
...
plt.tight_layout()
plt.savefig(f'{output_dir}/factor_ranking_by_altitude.png', dpi=300)
plt.show()

# --- B. 原有的分组计算逻辑继续 (保持不变或复用上述计算结果) ---
# ... 后面接原有的 B 以后部分 ...

# --- B. 分组计算 (按 Altitude 分组) ---
q_summary_list = []
interaction_list = []
risk_trends = []
...
for ele_val, ele_name in ele_map.items():
    ...
    q_dict['Altitude_Range'] = ele_name
    ...
    for col in features:
        ...
        for strata, val in mean_risk.items():
            risk_trends.append({'Altitude': ele_name, 'Factor': col, 'Strata': strata, 'Mean_Risk': val})

    # 3. 交互探测 (选当前组前两名)
    top_factors = pd.Series(q_dict).drop('Altitude_Range').astype(float).sort_values(ascending=False).head(3).index.tolist()
    ...
    for p1, p2 in pairs:
        ...
        interaction_list.append({
            'Altitude': ele_name, 'Factor_A': p1, 'Factor_B': p2,
...
# 1. 保存 Excel 结果
q_summary_df = pd.DataFrame(q_summary_list).set_index('Altitude_Range')
...
pivot_inter = inter_df.pivot_table(index="Altitude", columns=["Factor_A", "Factor_B"], values="q_Inter")
altitude_order = list(ele_map.values())
pivot_inter = pivot_inter.reindex(altitude_order).dropna(how="all")
sns.heatmap(pivot_inter, annot=True, cmap="YlOrRd", fmt=".3f")
plt.title('Interaction Detector Summary (q_Inter)')
